...
```dart
// pub: crypto ^3.0.0, uuid ^4.0.0
import 'dart:convert';
import 'package:crypto/crypto.dart';
import 'package:uuid/uuid.dart';

Map<String, String> sign(String method, String path, List<int> body, String secret) {
  final ts        = (DateTime.now().millisecondsSinceEpoch ~/ 1000).toString();
  final nonce     = const Uuid().v4();
  final bodyHash  = sha256.convert(body).toString();
  final sts       = '$method\n$path\n$ts\n$nonce\n$bodyHash';
  final secretKey = Hmac(sha256, utf8.encode(secret));
  final sig       = secretKey.convert(utf8.encode(sts)).toString();
  return {
    'X-Timestamp':  ts,
    'X-Nonce':      nonce,
    'X-Signature':  sig,
...
**Mobile (Flutter):**
  - Используйте пакет `device_info_plus`
  - Android: `AndroidDeviceInfo.id` (`Settings.Secure.ANDROID_ID`) — уникален для устройства + пользователя
  - iOS: `IosDeviceInfo.identifierForVendor` — сбрасывается при переустановке; для стабильности сохраняйте в `flutter_secure_storage` при первом запуске
  - Рекомендуемый паттерн для Flutter:

```dart
// pub: device_info_plus ^10.0.0, flutter_secure_storage ^9.0.0, uuid ^4.0.0
...
Future<String> getDeviceId() async {
  ...
  final cached  = await storage.read(key: 'device_id');
  ...
  final id = raw ?? const Uuid().v4();   // fallback: случайный UUID
